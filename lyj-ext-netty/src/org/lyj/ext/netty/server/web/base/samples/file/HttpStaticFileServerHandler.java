...
public class HttpStaticFileServerHandler
    ...
    public void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
        ...
        if (request.method() != GET) {
            ResponseUtil.sendError(ctx, METHOD_NOT_ALLOWED);
            return;
        }
        ...
        if (path == null) {
            ResponseUtil.sendError(ctx, FORBIDDEN);
            return;
        }

        File file = new File(path);
        if (file.isHidden() || !file.exists()) {
            ResponseUtil.sendError(ctx, NOT_FOUND);
            return;
        }
        ...
        if (file.isDirectory()) {
            if (uri.endsWith("/")) {
                ...
                ResponseUtil.sendRedirect(ctx, uri + '/');
            }
            return;
        }
        ...
        // Cache Validation
        String ifModifiedSince = request.headers().get(IHeaderNames.IF_MODIFIED_SINCE);
        if (ifModifiedSince != null && !ifModifiedSince.isEmpty()) {
            ...
            if (ifModifiedSinceDateSeconds == fileLastModifiedSeconds) {
                ResponseUtil.sendNotModified(ctx);
                return;
            }
        }
        ...
        if (HttpUtil.isKeepAlive(request)) {
            response.headers().set(IHeaderNames.CONNECTION, IHeaderValues.KEEP_ALIVE);
        }
    ...
    private static void sendListing(ChannelHandlerContext ctx, File dir) {
        ...
        response.headers().set(IHeaderNames.CONTENT_TYPE, "text/html; charset=UTF-8");
    ...
    private static void setDateAndCacheHeaders(HttpResponse response, File fileToCache) {
        ...
        response.headers().set(IHeaderNames.CACHE_CONTROL, "private, max-age=" + HTTP_CACHE_SECONDS);
