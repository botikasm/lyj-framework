package org.ly.server.application.controllers.messaging;
...
import org.ly.server.application.model.messaging.payloads.PayloadProgram;
import org.ly.server.application.programming.AppProgram;
...
public class MessageManager {
    ...
    private void handleRequestProgram(final OSEResponse response) throws Exception {
        final PayloadProgram payload = new PayloadProgram(response.request().payload());
        if (StringUtils.hasText(payload.namespace())
            ...
            final AppProgram program = ProgramsManager.instance().getNew(namespace);
